...
PLUGIN_PACKAGE_NAME="pulumi-resource-docker-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/pulumi/pulumi-docker/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-docker${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-docker"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-docker" \
