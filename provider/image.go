...
func (p *dockerNativeProvider) dockerBuild(ctx context.Context,
	...
	for scanner.Scan() {
		info, err := processLogLine(scanner.Text(), func(rm json.RawMessage) (bool, string, error) {
			...
			imageNameWithoutTag := reference.TrimNamed(imageName)
			repoDigest, err := reference.WithDigest(imageNameWithoutTag, digest)
			if err != nil {
	...
	// these should all be valid references
	// Must be a canonical ref: name@digest, not name:tag, or name:tag@digest
	...
	findImageID := func() (bool, error) {
		...
		for _, storedImage := range listResult {
			if storedImage.ID == imageID {
				...
				_ = p.host.LogStatus(ctx, "warning", urn,
					fmt.Sprintf("Image in store does not match built image, expected label %s=%s, found %s",
...
// error.
func processLogLine(msg string, onAuxMessage func(json.RawMessage) (bool, string, error)) (string, error) {
	var info string
	var jm jsonmessage.JSONMessage
	err := json.Unmarshal([]byte(msg), &jm)
	if err != nil {
		return "", fmt.Errorf("encountered error unmarshalling: %v", err)
	}
	if jm.Error != nil {
