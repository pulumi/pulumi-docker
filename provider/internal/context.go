...
func BuildxContext(contextPath, dockerfilePath string, namedContexts map[string]string) (string, error) {
	...
	if isLocalDir(fs, contextPath) {
		...
		fs := fsutil.NewFS(contextPath, &fsutil.WalkOpt{ExcludePatterns: excludes})
	...
	for _, namedContext := range namedContexts {
		if isLocalDir(fs, namedContext) {
			fs := fsutil.NewFS(namedContext, &fsutil.WalkOpt{ExcludePatterns: excludes})
			if _, err := hashPath(h, fs); err != nil {
				return "", err
			}
		}
...
func hashPath(h hash.Hash, fs fsutil.FS) (string, error) {
	err := fs.Walk(context.Background(), func(filePath string, fileInfo os.FileInfo, err error) error {
		if err != nil {
			return err
		}
		if fileInfo.IsDir() {
		...
		// the file to hash.
		return hashFile(h, fs, filePath, fileInfo.Mode())
	})
	if err != nil {
